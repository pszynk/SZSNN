...
__kernel void Const_rate_update(
				const ulong 			cycle,				//0
...
//	return;
	...
	/* parametry neuronu */
	__global const FLOAT_TYPE* gp_rate		= g_neuronParams_inPartition + p_RATE * neuronParamStep;

	__global FLOAT_TYPE* gv_last			= g_neuronVars_inPartition + v_LAST * neuronVarsStep;
	...
	for(unsigned i=0; i < l_partitionSize; i += THREADS_PER_GROUP)
	{
		...
		if (neuron <  l_partitionSize)
		{
			FLOAT_TYPE rate = gp_rate[neuron];
			FLOAT_TYPE last = gv_last[neuron] + 1;
			

			bool fired = (last == rate);
			...
			if (fired || firedExternal)
			{
				...
				l_fired[idx] = neuron;
#ifdef SZSNN_KERNEL_DEBUG
				printf("c%d[const_rate_update] p%dn%d fired (forced spike%d, current%d)\n",
