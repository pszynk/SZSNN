...
P = Population(n, IF_curr_exp, {'v_rest':-49, 'tau_syn_I': 10,'tau_syn_E': 5, 'v_reset':-100, 'v_thresh': -50, 'cm': 1.0, 'tau_m':30.0})
P.initialize('v', -70)
#P.initialize('Ie', 10)
Pe = P[:(n*0.8)]
Pi = P[(n*0.8):]
Ce = Projection(Pe, P, FixedProbabilityConnector(p_connect=0.01, allow_self_connections=False, weights=wei_E, delays=2), target='excitatory')
...
Ci = Projection(Pi, P, FixedProbabilityConnector(p_connect=0.01, allow_self_connections=False, weights=wei_I, delays=2), target='inhibitory') #
